...
// Send -
func (selfRef *AddPortMapping) Send(localPort, remotePort int, protocol string) bool {
	request := selfRef.buildRequest(localPort, remotePort, protocol)
...
func (selfRef *AddPortMapping) buildRequest(localPort, remotePort int, protocol string) *http.Request {
	//请求头
	header := http.Header{}
	...
	childList5 := Node{Name: "NewInternalClient", Content: selfRef.upnp.LocalHost}
...
func (selfRef *AddPortMapping) resolve(resultStr string) {
}
