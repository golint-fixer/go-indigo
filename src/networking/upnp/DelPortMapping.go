...
// Send -
func (selfRef *DelPortMapping) Send(remotePort int, protocol string) bool {
	request := selfRef.buildRequest(remotePort, protocol)
...
func (selfRef *DelPortMapping) buildRequest(remotePort int, protocol string) *http.Request {
	//请求头
	header := http.Header{}
...
func (selfRef *DelPortMapping) resolve(resultStr string) {
}
